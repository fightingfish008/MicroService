...
class CosBucket:
    ...
    def create_folder(self, dir_name):
        ...
        response, content = self.http.request(uri=self.url, method='POST', body='{"op": "create", "biz_attr": ""}',
    ...
    def list_folder(self, dir_name=None, prefix=None, num=1000, context=None):
        ...
        self.url = self.url.replace("<Region>", self.config.region).replace("<appid>", str(self.config.app_id)).replace(
    ...
    def query_folder(self, dir_name):
        ...
        self.url = 'http://' + self.config.region + '.file.myqcloud.com' + '/files/v2/' + str(
            self.config.app_id) + '/' + self.config.bucket + '/' + dir_name + '/?op=stat'
        self.headers['Authorization'] = CosAuth(self.config).sign_more(self.config.bucket, '', 30)
    ...
    def delete_folder(self, dir_name):
        ...
        if dir_name[0] == '/':
            dir_name = dir_name[1:len(dir_name)]
        self.url = 'http://' + self.config.region + '.file.myqcloud.com/files/v2/' + str(
            self.config.app_id) + '/' + self.config.bucket + '/' + dir_name + '/'
    ...
    def upload_file_stream(self, file, file_name, dir_name=None):
        """简单上传文件(https://www.qcloud.com/document/product/436/6066)
        ...
        # files = {'file': ('', open(real_file_path, 'rb'))}
        r = requests.post(url=self.url, data={'op': 'upload', 'biz_attr': '', 'insertOnly': '0'}, files={
            'filecontent': ('filecontent', file, 'application/octet-stream')}, headers=headers)
        return eval(r.content.decode('utf8')).get('data')
    ...
    def move_file(self, source_fileid, dest_fileid):
        ...
        self.url = "http://" + self.config.region + ".file.myqcloud.com/files/v2/" + str(
            self.config.app_id) + "/" + self.config.bucket + "/" + source_fileid
        headers = {}
        headers['Authorization'] = CosAuth(self.config).sign_once(self.config.bucket, source_fileid)
        r = requests.post(url=self.url, data={'op': 'move', 'dest_fileid': dest_fileid, 'to_over_write': '0'},
                          files={'filecontent': ('', '', 'application/octet-stream')}, headers=headers)
    ...
    def copy_file(self, source_fileid, dest_fileid):
        ...
        headers = {}
        headers['Authorization'] = CosAuth(self.config).sign_once(self.config.bucket, source_fileid)
        r = requests.post(url=self.url, data={'op': 'copy', 'dest_fileid': dest_fileid, 'to_over_write': '0'},
                          files={'filecontent': ('', '', 'application/octet-stream')}, headers=headers)
        code = eval(r.content.decode('utf8')).get('code')
        if code == 0:
            return True
